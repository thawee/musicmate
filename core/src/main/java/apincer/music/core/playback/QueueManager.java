...
public class QueueManager {
     ...
     */
    private final List<MediaTrack> queueList = new CopyOnWriteArrayList<>();
     ...
     *
     * @return A list of {@link MediaTrack} objects.
     */
    public List<MediaTrack> getSongs() {
    ...
    public synchronized void loadPlayingQueue() {
        try {
            ...
            for (int i = 0; i < queues.size(); i++) {
                MediaTrack track = queues.get(i).getTrack();
                queueList.add(track);
     ...
     */
    public synchronized void setPlaybackTrack(MediaTrack track) {
        if (track == null) return;
     ...
     *
     * @return The next {@link MediaTrack}, or {@code null} if the end of the queue is reached.
     */
    public MediaTrack getNextTrack() {
    ...
    public MediaTrack getPreviousTrack() {
        int baseIndex = (playbackIndex != -1) ? playbackIndex : currentIndex;
