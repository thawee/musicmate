...
/**
 * Handles incoming WebSocket commands from the web UI client and generates appropriate
 * responses based on the music library, playback state, and external metadata services.
 * This class acts as the bridge between the WebSocket server and the core application logic
 * (database, playback service, etc.).
 */
public class WebSocketContent {
     ...
     *
     * @param context         The Android application context, needed for waveform generation.
     * @param playbackService The service managing music playback and UPnP/DLNA targets.
     * @param tagRepos        The repository for accessing music library data (tags, queue).
     */
        ...
        if (rendererList != null && !rendererList.isEmpty()) {
            renderers = rendererList.stream()
                    .map(device -> Map.<String, Object>of(
                            ...
                            // DO NOT add 'active' here - use getPlaybackTarget for active status
                    ))
                    .collect(Collectors.toList());
        }
