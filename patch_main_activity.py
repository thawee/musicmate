...
if start_idx == -1:
    ...
    if end_idx != -1:
            ...
            (isDeep, updatedDirs) -> { // onScan
                ...
                if (isDeep) {
                    androidx.preference.PreferenceManager.getDefaultSharedPreferences(getApplicationContext())
                            .edit().putLong("PREF_LAST_SCAN_TIME", 0)
                            .apply();
                }
                TagRepository.saveDirectories(MainActivity.this, updatedDirs);
                viewModel.scanMusic(getApplicationContext(), true);
            },
            (sid) -> { // onAddStorage
                ...
                dialog.setDialogSelectionListener(files -> {
                    if (files != null && files.length > 0) {
                        ...
                        dirs.add(f);
                        if(alertHolder[0] != null) alertHolder[0].dismiss();
                        TagRepository.saveDirectories(MainActivity.this, dirs); // Save immediately
        ...
        if (alert.getWindow() != null) {
            alert.getWindow().setBackgroundDrawable(new android.graphics.drawable.ColorDrawable(Color.TRANSPARENT));
        }
