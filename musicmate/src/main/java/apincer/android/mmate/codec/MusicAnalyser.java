   ...
   // private double resampledScore = 0.0;
    ...
    public double calculateDRMeter(byte[] audioData) {
        ...
        for (int ch = 0; ch < channels; ch++) {
            ...
            if (blocksForChannel.isEmpty()) continue; // Simplified check
    ...
    public static float[] generateWaveformOld(byte[] pcmAudioData, int points) {
        ...
        // Create a ShortBuffer to easily iterate over audio samples
        final ShortBuffer samples = ByteBuffer.wrap(pcmAudioData)
                .order(ByteOrder.LITTLE_ENDIAN) // Common for WAV files
                .asShortBuffer();
        ...
        for (int i = 0; i < points; i++) {
            int startSample = i * samplesPerPoint;
            int endSample = startSample + samplesPerPoint;

            // Find the peak amplitude within this block of samples
            short peakInBlock = 0;
            for (int s = startSample; s < endSample && s < numSamples; s++) {
                short currentSample = samples.get(s);
                if (Math.abs(currentSample) > peakInBlock) {
                    peakInBlock = (short) Math.abs(currentSample);
                }
            }

            waveform[i] = peakInBlock;

            // Update the overall max amplitude
            if (peakInBlock > maxAmplitude) {
                maxAmplitude = peakInBlock;
            }
        }
