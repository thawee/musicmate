...
public class WebDAVContextHandler implements HTTPServer.ContextHandler {
    ...
    public static String XML_HEAD = "<?xml version=\"1.0\" encoding=\"utf-8\" ?><D:multistatus xmlns:D=\"DAV:\">";
    public static String XML_FOOT = "</D:multistatus>";
    private Context context;
    protected final DateFormat dateFormatter;
    public WebDAVContextHandler(Context applicationContext) {
    ...
    private void serveArtistContent(ContextPath path, HTTPServer.Request req, HTTPServer.Response resp) throws IOException {
        ...
        if(isDavRequest(req)) {
            ...
            String content = createDavIndex(path.artist, null,list, req.getPath(), uri);
    ...
    private void serveGroupingContent(ContextPath path, HTTPServer.Request req, HTTPServer.Response resp) throws IOException {
        ...
        if(isDavRequest(req)) {
            String uri =  buildUri(req);
            String content = createDavIndex(path.grouping, list,null, req.getPath(), uri);
            resp.send(Status.MULTI_STATUS.getRequestStatus(), content);
        }else {
            // get file content
            // send playing event
    ...
    private String createDavIndex(String name, List<String> dirs,List<MusicTag> files, String path,String uri) {
        ...
        if(files != null) {
            for(MusicTag file: files) {
                String fileuri = uri + getFilename(file);
    ...
    private String createHtmlIndex(List<String> dirs, List<MusicTag> files, String path, boolean linkToParent) {
        ...
        fh.format(HTML_HEADER_START, path);
            ...
            if (files != null) {
                for (MusicTag file : files) {
                    try {
                        String name =  getFilename(file);  // id_title.ext
    ...
    private String getFilename(MusicTag tag) {
        return tag.getId() + "_" + tag.getTitle()+"."+tag.getFileFormat();
    }
    ...
    private String createDavResponse(String name, String date, String uri)   {
        ...
        try {
            ...
            content = "<D:response>" +
                    ...
                    "</D:prop>" +
                    "<D:status>" + Status.OK.getDescription() + "</D:status>" +
                    "</D:propstat>" +
                    "</D:response>";
        }catch (Exception ex) {}
        return content;
    ...
    private String createDavResponse(MusicTag tag, String uri)  {
        ...
        try {
            ...
            content = "<D:response>" +
                            ...
                            "<D:prop>" +
                            "<D:creationdate>" + date + "</D:creationdate>" +
                            "<D:displayname>" + trimToEmpty(tag.getTitle()) + "</D:displayname>" +
