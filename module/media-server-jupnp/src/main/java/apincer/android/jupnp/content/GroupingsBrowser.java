...
public class GroupingsBrowser extends AbstractContentBrowser {
    public GroupingsBrowser(Context context, TagRepository tagRepos) {
    ...
    public Integer getTotalMatches(ContentDirectory contentDirectory, String myId) {
        return getOrmLite().getGrouping().size(); // +1; // +1 for Downloads folder
    }
    ...
    public List<Container> browseContainer(ContentDirectory contentDirectory, String myId, long firstResult, long maxResults, SortCriterion[] orderby) {
        ...
        List<MusicFolder> groupings = getOrmLite().getGroupingWithChildrenCount();
