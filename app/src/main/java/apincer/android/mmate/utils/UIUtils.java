...
public class UIUtils  {
    ...
    public static void buildStoragesUsed(Application application, LinearLayout vStoragesLayout, Map<String, Long> actualSize, Map<String, Long> estimatedSize) {
        ...
        for(String sid: storageIds) {
            ...
            String storageName = StorageId.PRIMARY.equals(sid) ? "Internal" : "SD Card";
            actualInfo.setText(formatCompactStorageText(storageName, used / (1024.0 * 1024.0 * 1024.0), total / (1024.0 * 1024.0 * 1024.0), pcnt));

            if (actualProgressBar != null) {
                actualProgressBar.setMax(100);
                actualProgressBar.setProgress((int) Math.round(pcnt));
                actualProgressBar.setProgressTintList(ColorStateList.valueOf(barColor));
            }
    ...
    public static void buildStoragesStatus(Application application, LinearLayout vStoragesLayout) {
        ...
        for (String sid : storageIds) {
            ...
            String storageName = StorageId.PRIMARY.equals(sid) ? "Internal" : "SD Card";
            info.setText(formatCompactStorageText(storageName, used / (1024.0 * 1024.0 * 1024.0), total / (1024.0 * 1024.0 * 1024.0), pcnt));

            if (progressBar != null) {
                progressBar.setMax(100);
                progressBar.setProgress((int) Math.round(pcnt));
    ...
    private static String formatCompactStorageText(String label, double usedGB, double totalGB, double pcnt) {
        String usedStr = (usedGB >= 100 || usedGB % 1 == 0) ? String.format(Locale.getDefault(), "%.0f", usedGB) : String.format(Locale.getDefault(), "%.1f", usedGB);
        String totalStr = (totalGB >= 100 || totalGB % 1 == 0) ? String.format(Locale.getDefault(), "%.0f", totalGB) : String.format(Locale.getDefault(), "%.1f", totalGB);
        return String.format(Locale.getDefault(), "%s: %s/%s GB (%d%%)", label, usedStr, totalStr, Math.round(pcnt));
    }
