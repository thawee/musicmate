...
public class MediaServerImpl implements MediaServer {
    ...
    public MediaServerImpl(Context context, UpnpServiceConfiguration upnpServiceCfg, FileRepository fileRepos, TagRepository tagRepos) {
    ...
    private synchronized void addRenderer(RemoteDevice device) {
        ...
        String host = getDeviceIpAddress(device);
        String name = device.getDetails().getFriendlyName();
        RendererDevice renderer = new RendererDevice(udn, host, name);
        RendererDevice udnDev = getRendererByUDN(udn);
        if(udnDev != null) {
    ...
    public RendererDevice getRendererByUDN(String udn) {
        for(RendererDevice dev: renderers) {
            String ludn = dev.getUdn();
    ...
    public RendererDevice getRendererByIpAddress(String ipAddress) {
        for(RendererDevice dev: renderers) {
            if(ipAddress.equals(dev.getHost())) {
                return dev;
            }
        }
        return null;
    ...
    private static String getDeviceIpAddress(RemoteDevice device) {
    ...
    @Override
    public void getCurrentSong(String rendererUdn) {
        Device device = upnpService.getRegistry().getDevice(new UDN(rendererUdn), false);
        if (device == null) {
            return;
        }
        ...
        upnpService.getControlPoint().execute(new GetMediaInfo(avTransportService) {
            ...
            public void received(ActionInvocation invocation, MediaInfo mediaInfo) {
                ...
                getPlaybackService().setNowPlaying(song);
            ...
            final Runnable positionInfoPoller = () -> getPositionInfo(rendererUdn, new PositionInfoCallback() {
                ...
                public void onReceived(PositionInfo positionInfo) {
                   ...
                   //     nowPlaying.setElapsed(parseHmsToSeconds(positionInfo.getRelTime()));
                        getPlaybackService().setNowPlayingElapsedTime(parseHmsToSeconds(positionInfo.getRelTime()));
                       // nowPlayingSubject.onNext(nowPlaying);
